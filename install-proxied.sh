...
install_s-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/kunori-kiku/s-ui-modified/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        wget -N --no-check-certificate -O /tmp/s-ui-linux-$(arch).tar.gz https://gh-proxy.com/github.com/kunori-kiku/s-ui-modified/releases/download/${last_version}/s-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://gh-proxy.com/github.com/kunori-kiku/s-ui-modified/releases/download/${last_version}/s-ui-linux-$(arch).tar.gz"
    ...
    echo -e "You cannot access the panel directly."
    echo -e "You may access the Panel following the instructions:${green}"
