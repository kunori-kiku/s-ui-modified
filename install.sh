...
install_s-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /tmp/s-ui-linux-$(arch).tar.gz https://github.com/kunori-kiku/s-ui-modified/releases/download/${last_version}/s-ui-linux-$(arch).tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/kunori-kiku/s-ui-modified/releases/download/${last_version}/s-ui-linux-$(arch).tar.gz"
